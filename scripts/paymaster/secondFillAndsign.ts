...
async function main() {
  ...
  function getAccountInitCode () {
    return hexConcat([ //hexConcat은 hex형태로 concat하도록 도와주는 함수 이더스의 라이브러리에 있다.
      ...
      accountFactory.interface.encodeFunctionData('createAccount', [sender.address, 0]) //지갑을 생성할 주소와 salt값이 여기에 해당
    ])
  }
  ...
  let userOp2nd : UserOperation = {
    sender : sender.address,
    nonce : 1, //지갑의 nonce와 동일
    initCode : '0x', //처음에 지갑이 생성될 때 initCode가 필요, 이후에는 0x를 넣어서 트랜잭션을 보내면 된다.
    callData : transferCalldata, //실제로 실행시킬 트랜잭션
    callGasLimit : gasEtimated,
    verificationGasLimit,
    preVerificationGas : 210000,
    maxFeePerGas,
    maxPriorityFeePerGas : 1e9,
    paymasterAndData : ethers.utils.keccak256(PayMasterAddress), // entry point에서 _copyUserOpToMemory 를 호출했을때             mUserOp.paymaster = address(bytes20(paymasterAndData[: 20])); 값에서 주소가 나와야 한다.
    signature : '0x'//원하는 서명을 넣으면 된다. TokenPaymaster 예제 에서는 따로 서명이 필요하지는 않다. Paymaster의 _validatePaymasterUserOp 를 체크하면 된다.
  }

  console.log("userOp2nd :", userOp2nd)
  ...
  const postOp = await entryPoint.handleOps([userOp2nd], beneficiary.address, {gasLimit: 1e7}).then(async tx => console.log(tx))
